...
class CalibSetup:
    """
    Structure for storing calibration experimental data, likelihood,
    discrepancy, etc. Includes the following methods:
    ...
    * setClusterPriors
    ...
    def __init__(self, bounds, constraint_func="bounds"):
        ...
        :param bounds: dictionary where keys are parameter names and items are
                       tuples that are the lower and upper bounds for the
                       parameters
        :param constraint_func: a function that takes a dictionary of
                                parameter combinations as well as the dictionary
                                of bounds and (these should have matching keys)
                                and returns a vector of 1s and 0s with 1s where
                                the parameter combinations meet the constraint.
                                Alternatively, if no constraints other than
                                bounds exist, passing "bounds" for this argument
                                will use the proper constraint function.
        """
    ...
    def addVecExperiments(
        ...
        """
        Add an experiment (really a data/model combination), or a set of
        experiments for whichmodel prediction of the quantity of interest
        is vectorized.

        :param yobs: a vector (numpy array) of observed data
        :param model: a class with an eval method.  The eval method should
                      include arguments parmat (a dictionary with keys
                      matching those in the bounds dictionary, and items
                      that are parameter combinations), pool (logical
                      indicating whether this is a calculation for a pooled
                      model or a hierarchical model), and nugget (logical
                      indicating whether prediction is done including a
                      nugget term) and should return the model evaluations
                      at the parameter combinations.  If this is an emulator
                      with posterior samples, a step method can also be
                      included
        :param sd_est: a list or numpy array of initial values for observation
                       noise standard deviation, len(sd_est) = number of
                       separately-estimated s2 values
        :param s2_df: a list or numpy array of initial values for s2 Inverse
                      Gamma prior degrees of freedom (s2_df = 0; Half-Cauchy
                      prior), same structure as sd_est
        :param s2_ind: a list or numpy array of indices for s2 value
                       associated with each element of yobs,
                       len(s2_ind) = len(yobs), max(s2_ind)+1 = len(sd_est)
        :param meas_error_cor: (optional) correlation matrix for observation
                               measurement errors, default = independent
        :param theta_ind: a list or numpy array of indices for theta_i
                          associated with each element of yobs (usually,
                          indexes experiments), len(theta_ind) = len(yobs)
        :param D: (optional) numpy array containing basis functions for
                  discrepancy, possibly including intercept.
                  D.shape = (length of yobs, number of bases)
        :param discrep_tau: (optional) fixed prior variance for discrepancy
                            basis coefficients
                            (discrepancy = D @ discrep_vars, discrep_vars ~ N(0,discrep_tau))
        """
    ...
    def setTemperatureLadder(self, temperature_ladder, start_temper=1000):
        ...
        :param temperature_ladder : numpy array of increasing temperatures all
                                    above 1, e.g., 1.05**np.arange(50)
        :param start_temper : (optional) MCnC iteration at which to start the
                              parallel tempering, default = 1000
        """
    ...
    def setMCMC(
        ...
        :param start_var_theta : (optional) initial variance of adaptive MCMC
                                proposal distributions for theta. Can be
                                increased from default if posterior samples of
                                theta are stuck at a single value across many
                                iterations
        :param start_tau_theta : (optional) np.exp(start_tau_theta) is the
                                 initial scaling factor for the adaptive MCMC
                                 proposal covariance for theta. Can be kept at
                                 default for most users.
        :param start_var_ls2 : (optional) initial variance of adaptive MCMC
                               proposal distributions for log(s2), i.e. the
                               log of the observation error/noise standard
                               deviation. Can be increased from default if
                               posterior samples of theta are stuck at a
                               single value across many iterations
        :param start_tau_ls2 : (optional) np.exp(start_tau_ls2) is the initial
                               scaling factor for the adaptive MCMC proposal
                               covariance for log(s2). Can be kept at default
                               for most users.
        :param start_adapt_iter : (optional) MCMC iteration at which to start
                                  adapting the MCMC proposal distributions. Can
                                  be left as default for most users.
        """
        self.nmcmc = nmcmc
        self.nburn = nburn
        self.thin = thin
    ...
    def setHierPriors(self, theta0_prior_mean, theta0_prior_cov, Sigma0_prior_df, 
                      Sigma0_prior_scale):
        """
        Define hierachical model hyperparameters, where
        theta_i ~ N(theta0, Sigma0)
        with priors (1)
        theta0~N(mean=theta0_prior_mean,covariance=theta0_prior_cov)
        and
        (2) Sigma0~InverseWishart(V=Sigma0_prior_scale,m=Sigma0_prior_df)
        where prior
        E(Sigma0)=V/(m-p-1) and Var(Sigma0)ii = 2 (Vii)^2/(((m-p-1)^2)*(m-p-3))

        :param theta0_prior_mean : numpy array of length self.p containing
                                   initial values for calibration parameter
                                   theta0, usually np.repeat(0.5, self.p)
        :param theta0_prior_cov : numpy array (self.p by self.p) containing
                                  the prior covariance for theta0, usually
                                  np.eye(self.p)*user_defined_prior_variance
        :param Sigma0_prior_df :  prior degrees of freedom for the prior for
                                  Sigma0, at least 1 + self.p, where larger
                                  values generally indicate theta_i values
                                    closer to theta_0
        :param Sigma0_prior_scale : prior scale for the prior for Sigma0,
                                    where smaller values generally indicate
                                    theta_i values closer to theta_0
        """
    ...
    def setClusterPriors(self, nclustmax=None, eta_prior_shape=2, eta_prior_rate=0.1):
        ...
        :param nclustmax : maximum number of unique theta values to estimate
                           (i.e., maximum number of clusters)
...
initfunc_probit = np.random.normal
...
def chol_sample(mean, cov):
    out = mean + np.dot(np.linalg.cholesky(cov), 
                        np.random.standard_normal(mean.size))
    return out


def chol_sample_1per(means, covs):
    out = means + np.einsum("tnpq,tnq->tnp", cholesky(covs), 
                            normal(size=means.shape))
    return out


def chol_sample_nper(means, covs, n):
    out = means + np.einsum("ijk,ilk->ilj", 
                            cholesky(covs), 
                            normal(size=(*means.shape, n)))
    return 


def chol_sample_1per_constraints(means, covs, cf, bounds_mat, bounds_keys, 
                                 bounds):
    ...
    while np.any(~good):
        cand[np.where(~good)] = +means[~good] + np.einsum(
            "ijk,ik->ij", chols[~good], normal(size=((~good).sum(), 
                                                     means.shape[1]))
        )
        good[~good] = cf(tran_unif(cand[~good], bounds_mat, bounds_keys), 
                         bounds)
    return cand


def chol_sample_nper_constraints(means, covs, n, cf, bounds_mat, bounds_keys, 
                                 bounds):
    """Sample with constraints.  If fail constraints, resample."""
    chols = cholesky(covs)
    ...
    for i in range(cand.shape[0]):
        ...
        while np.any(~goodi):
            cand[i, np.where(~goodi)[0]] = +means[i] + np.einsum(
                "ik,nk->ni", chols[i], normal(size=((~goodi).sum(), 
                                                    means.shape[1]))
            )
            goodi[np.where(~goodi)[0]] = cf(
                tran_unif(cand[i, np.where(~goodi)[0]], bounds_mat, 
                          bounds_keys), bounds
            )
    return cand
...
def invwishart_logpdf(w, df, scale):  # VALIDATED
    ...
    ld = (
        ...
        - 0.5
        * np.einsum("...ii->...", np.einsum("ji,...ij->...ij", scale, 
                                            np.linalg.inv(w)))
    )
    return ld
...
def invgamma_logpdf(s, alpha, beta):
    """log pdf of inverse gamma distribution -- 
       Assume s = (n x p); alpha, beta = (p)"""
    ld = (
        +alpha * np.log(beta) - gammaln(alpha) - (alpha - 1) * np.log(s) - beta / s
    ).sum(axis=1)
    return ld
...
def gamma_logpdf(s, alpha, beta):
    ...
    ld = +alpha * np.log(beta) - gammaln(alpha) + (alpha - 1) * np.log(s) - beta * s
    return ld
...
class AMcov_pool:
    def __init__(self, ntemps, p, start_var=1e-4, start_adapt_iter=300, tau_start=0.0):
        self.eps = 1.0e-12
    ...
    def update(self, x, m):
        if m > self.start_adapt_iter:
            ...
            self.cov = +((m - 1) / m) * self.cov + ((m - 1) / (m * m)) * np.einsum(
                "ti,tj->tij", x[m - 1] - self.mu, x[m - 1] - self.mu
            )
            self.S = self.AM_SCALAR * np.einsum(
                "ijk,i->ijk", self.cov + np.eye(self.p) * self.eps, np.exp(self.tau)
            )
    ...
    def update_tau(self, m):
        ...
        if (m % 100 == 0) and (m > self.start_adapt_iter):
            ...
            self.count_100 *= 0
            # note, e^tau scales whole covariance matrix, so it shrinks 
            # covariance for inert inputs too much...need decor for those.
...
class AMcov_hier:
    ...
    def update(self, x, m):  # called in mth iteration,
        if m > self.start_adapt_iter:
...
def calibHier(setup):
    ...
    s2_ind_mat = [
        (setup.s2_ind[i][:, None] == range(setup.ns2[i])) for i in range(setup.nexp)
    ]
    s2_which_mat = [
    ...
    while np.any(~good):
        ...
        good[np.where(~good)] = setup.checkConstraints(
            tran_unif(
                theta0_start[np.where(~good)], setup.bounds_mat, setup.bounds.keys()
            ),
            setup.bounds,
        )
    ...
    itl_mat = [  # matrix of temperatures for use with alpha calculation
    ...
    for i in range(setup.nexp):
        ...
        for t in range(setup.ntemps):
            ...
            for j in range(setup.ntheta[i]):
                ...
                # right now, assuming for vectorized models that new theta 
                # means new s2. if you wanted to have multiple s2 for one 
                # theta, you would have to update thetas jointly or sequentially 
                # (not independently), unless working with diagonal
                # many possible cases, for now make it work for strength 
                # project, generalize later
                ...
                # this isnt getting nthetas correct, probably need to change 
                # models script... there should be a separate likelihood 
                # evaluation (with separate covariance) for every i, t, ntheta. 
                # In diagonal case, we could vectorize over t ntheta...
                # for now, break into separate calls.  Later may be worthwhile 
                # to try to vectorize more.
    ...
    theta0_prior_cov = setup.theta0_prior_cov  
    ...
    count_decor = [
        np.zeros((setup.ntemps, setup.ntheta[i], setup.p)) for i in range(setup.nexp)
    ]
    count_decor2 = np.zeros((setup.ntemps, setup.p))
    count_s2 = np.zeros([setup.nexp, setup.ntemps], dtype=int)

    theta_cand = [
        np.empty([setup.ntemps, setup.ntheta[i], setup.p]) for i in range(setup.nexp)
    ]
    theta_cand_mat = [
        np.empty([setup.ntemps * setup.ntheta[i], setup.p]) for i in range(setup.nexp)
    ]
    ...
    alpha = [
        np.ones((setup.ntemps, setup.ntheta[i])) * -np.inf for i in range(setup.nexp)
    ]
    ...
    for m in pbar.pbar(range(1, setup.nmcmc)):
        ...
        for i in range(setup.nexp):
            ...
            if setup.models[i].stochastic:  # update emulator
                pred_curr[i] = setup.models[i].eval(
                    tran_unif(
                        theta[i][m].reshape(setup.ntemps * setup.ntheta[i], setup.p),
                        setup.bounds_mat,
                        setup.bounds.keys(),
                    ),
                    pool=False,
        ...
        for i in range(setup.nexp):
            ...
            alpha[i][good_values[i]] = itl_mat[i][good_values[i]] * (
                ...
                + mvnorm_logpdf_(
                    theta_cand[i], theta0[m - 1], Sigma0_inv_curr, Sigma0_ldet_curr
                )[good_values[i]]
                - mvnorm_logpdf_(
                    theta[i][m - 1], theta0[m - 1], Sigma0_inv_curr, Sigma0_ldet_curr
                )[good_values[i]]
            )
            # MCMC Accept
        ...
        for i in range(setup.nexp):
            ...
            if setup.models[i].s2 == "gibbs":
                ...
                # M-H update s2
                # NOTE: there is something wrong with this...with no tempering, 
                # 10 kolski experiments, reasonable priors, s2 can diverge for 
                # some experiments (not a random walk, has weird patterns).
                # This seems to be because of the joint update, but is strange.  
                # Could be that individual updates
                ...
                for t in range(setup.ntemps):
                    ...
                    for j in range(setup.ntheta[i]):
                        marg_lik_cov_candi[t][j] = setup.models[i].lik_cov_inv(
                            np.exp(ls2_candi[t, setup.s2_ind[i]])[setup.s2_ind[i] == j]
                ...
                alpha_s2 += setup.itl * setup.s2_prior_kern[i](
        ...
        if m % setup.decor == 0:
            for k in range(setup.p):
                ...
                good_values_theta0 = setup.checkConstraints(
                    tran_unif(theta0_cand, setup.bounds_mat, setup.bounds.keys()),
                    setup.bounds,
                )
                ...
                for i in range(setup.nexp):
                    ...
                    good_values[i][:] = (
                        good_values_mat[i].reshape(setup.ntemps, setup.ntheta[i]).T
                        * good_values_theta0
                    ).T
                    pred_cand[i][:] = setup.models[i].eval(
                        tran_unif(
                    ...
                    alpha[i][good_values[i]] = itl_mat[i][good_values[i]] * (
                        ...
                        - mvnorm_logpdf_(
                            theta[i][m], theta0[m], Sigma0_inv_curr, Sigma0_ldet_curr
                        )[good_values[i]]
                    )
                # now sum over alpha (for each temperature), 
                # add alpha for theta0 to prior, accept or reject
                alpha_tot = (
                    sum(alpha).T
                    - 0.5
                    * setup.itl
                ...
                for i in range(setup.nexp):
                    sw_alpha[:] = sw_alpha + (
                        ...
                        +setup.s2_prior_kern[i](
                            np.exp(log_s2[i][m][sw.T[0]]), setup.ig_a[i], setup.ig_b[i]
                        ).sum(axis=1)
                        + mvnorm_logpdf_(
                            theta[i][m][sw.T[0]],
                            theta0[m, sw.T[0]],
                        ...
                        # for t_1
                        - setup.s2_prior_kern[i](
                            np.exp(log_s2[i][m][sw.T[1]]), setup.ig_a[i], setup.ig_b[i]
                        ).sum(axis=1)
                        - mvnorm_logpdf_(
                            theta[i][m][sw.T[1]],
                            theta0[m, sw.T[1]],
...
def calibPool(setup):
    ...
    log_s2 = [
        np.ones([setup.nmcmc, setup.ntemps, setup.ns2[i]]) for i in range(setup.nexp)
    ]
    ...
    good = setup.checkConstraints(
        tran_unif(theta_start, setup.bounds_mat, setup.bounds.keys()), setup.bounds
    )
    while np.any(~good):
        theta_start[np.where(~good)] = initfunc_unif(size=[(~good).sum(), setup.p])
        good[np.where(~good)] = setup.checkConstraints(
            tran_unif(
                theta_start[np.where(~good)], setup.bounds_mat, setup.bounds.keys()
            ),
            setup.bounds,
        )
    theta[0] = theta_start
    ...
    itl_mat = [  # matrix of temperatures for use with alpha calculation
        (np.ones((setup.ns2[i], setup.ntemps)) * setup.itl).T for i in range(setup.nexp)
    ]
    ...
    for m in pbar.pbar(range(1, setup.nmcmc)):
        ...
        if m % setup.decor == 0:
            for k in range(setup.p):
                ...
                alpha[good_values] = setup.itl[good_values] * (
                    llik_diff
                )  
                for t in np.where(np.log(uniform(size=setup.ntemps)) < alpha)[0]:
        ...
        if m > setup.start_temper and setup.ntemps > 1:
            for _ in range(setup.nswap):
                ...
                for i in range(setup.nexp):
                    sw_alpha += (setup.itl[sw.T[1]] - setup.itl[sw.T[0]]) * (
                        setup.s2_prior_kern[i](
                            np.exp(log_s2[i][m][sw.T[0]]), setup.ig_a[i], 
                            setup.ig_b[i]
                        ).sum(axis=1)
                        - setup.s2_prior_kern[i](
                            np.exp(log_s2[i][m][sw.T[1]]), setup.ig_a[i], 
                            setup.ig_b[i]
                        ).sum(axis=1)
                    )
                ...
                for tt in sw[
                    ...
                    for i in range(setup.nexp):
                        ...
                        marg_lik_cov_curr[i][tt[0]], marg_lik_cov_curr[i][tt[1]] = (
