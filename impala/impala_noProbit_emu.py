...
class CalibSetup:
    ...
    * setClusterPriors
    ...
    def setHierPriors(
        self, theta0_prior_mean, theta0_prior_cov, Sigma0_prior_df, Sigma0_prior_scale
    ):
        """
...
def chol_sample_1per_constraints(means, covs, cf, bounds_mat, bounds_keys, bounds):
    ...
    while np.any(~good):
        cand[np.where(~good)] = +means[~good] + np.einsum(
            "ijk,ik->ij", chols[~good], normal(size=((~good).sum(), means.shape[1]))
        )
    ...
    for i in range(cand.shape[0]):
        ...
        while np.any(~goodi):
            cand[i, np.where(~goodi)[0]] = +means[i] + np.einsum(
                "ik,nk->ni", chols[i], normal(size=((~goodi).sum(), means.shape[1]))
            )
            goodi[np.where(~goodi)[0]] = cf(
                tran_unif(cand[i, np.where(~goodi)[0]], bounds_mat, bounds_keys), bounds
            )
    return cand
...
def invwishart_logpdf(w, df, scale):  # VALIDATED
    ...
    ld = (
        ...
        - 0.5
        * np.einsum("...ii->...", np.einsum("ji,...ij->...ij", scale, np.linalg.inv(w)))
    )
    return ld
...
def calibHier(setup):
    ...
    for m in pbar.pbar(range(1, setup.nmcmc)):
        ...
        if False:  # m % setup.decor == 0:
            for i in range(setup.nexp):
                for k in range(setup.p):
                    ...
                    # Calculate log-probability of MCMC Accept
                    alpha[i][:] = -np.inf
                    alpha[i][good_values[i]] = -0.5 * itl_mat[i][good_values[i]] * (
        ...
        if False:
            ...
            ls2_cand = [
                cov_ls2_cand[i].gen_cand(log_s2[i], m) for i in range(setup.nexp)
            ]
            ...
            for i in range(setup.nexp):
                ...
                for t in range(setup.ntemps):
                    ...
                    for j in range(setup.ntheta[i]):
                        marg_lik_cov_cand[i][t][j] = setup.models[i].lik_cov_inv(
