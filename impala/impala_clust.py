...
def bincount2D_vectorized(a, max_count):
    ...
    return np.bincount(a_offs.ravel(), minlength=a.shape[0] * max_count).reshape(
        -1, max_count
    )
...
def sample_delta_per_temperature(curr_delta_t, njs_t, log_posts_t, inv_temp_t, eta_t):
    ...
    for s in range(curr_delta_t.shape[0]):
        ...
        temp[:] = -np.inf
        temp[(njs_t + djs) > 0] = inv_temp_t * (  # log-posterior under each cluster
            np.log((njs_t + djs)[njs_t + djs > 0]) + log_posts_t.T[s, njs_t + djs > 0]
        )
        temp[:] -= temp.max()  #
...
def calibClust(setup, parallel=False):
    ...
    good = setup.checkConstraints(
        impa.tran_unif(theta0_start, setup.bounds_mat, setup.bounds.keys()),
        setup.bounds,
    )
    while np.any(~good):
        theta0_start[np.where(~good)] = impa.initfunc_unif(
    ...
    delta_ind_mat = [
        (delta[i][0, :, :, None] == range(setup.nclustmax)) for i in range(setup.nexp)
    ]
    ...
    theta_unravel = [
        np.repeat(np.arange(setup.ntemps), setup.ntheta[i]) for i in range(setup.nexp)
    ]
    # initialize Theta
    ...
    for i in range(setup.nexp):
        theta_hist[i][0] = theta[0, theta_unravel[i], delta[i][0].ravel()].reshape(
            setup.ntemps, setup.ns2[i], setup.p
        )
    ...
    llik_curr = [
        None
    ] * setup.nexp  # [i], ntheta[i] x ntemps ### was [i] x [ntemps, ns2[i]]
    ...
    marg_lik_cov_curr = [
        None
    ] * setup.nexp  # does not need to get summed over experiments (should be one for each experiment, regardless of cluster)
    for i in range(setup.nexp):
        pred_curr[i] = setup.models[i].eval(
            impa.tran_unif(
                theta_hist[i][0].reshape(-1, setup.p),
                setup.bounds_mat,
                setup.bounds.keys(),
            ),
            False,
        ...
        pred_cand_delta[i] = (
            ...
            .eval(
                impa.tran_unif(
                    theta[0].reshape(-1, setup.p), setup.bounds_mat, setup.bounds.keys()
                ),
                True,
            )
        ...
        for t in range(setup.ntemps):
            ...
            for j in range(setup.ntheta[i]):
                for k in range(setup.nclustmax):
                    llik_cand_delta[i][t][k][j] = setup.models[i].llik(
                        setup.ys[i][theta_which_mat[i][j]],
                        pred_cand_delta[i][t][k][theta_which_mat[i][j]],
                        marg_lik_cov_curr[i][t][j],
                    )
    ...
    for m in range(1, setup.nmcmc):
        ...
        for i in range(setup.nexp):
            pred_cand_delta[i][:] = (
                ...
                .reshape(setup.ntemps, setup.nclustmax, setup.y_lens[i])
            )  ### isnt this just pred_curr??? Speedup.~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~``

            llik_cand_delta[i] = np.empty(
                [setup.ntemps, setup.nclustmax, setup.ntheta[i]]
            )
            for t in range(setup.ntemps):
                for j in range(setup.ntheta[i]):
                    for k in range(setup.nclustmax):
            ...
            for j in range(setup.ntheta[i]):
                ...
                with np.errstate(divide="ignore", invalid="ignore"):
                    cluster_cum_prob[:] = +np.log(
                        clust_mem_count + djs
                    ) + llik_cand_delta[  # this line doesnt match paper? ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
                        i
                    ][
                        :, :, j
                    ] * setup.itl.reshape(
                ...
                delta[i][m, :, j] = (
                    cluster_sample_unif[i][:, j].reshape(-1, 1) > cluster_cum_prob
                ).sum(axis=1)
        ...
        for i in range(setup.nexp):
            ...
            for t in range(setup.ntemps):
                for j in range(setup.ntheta[i]):
                    ...
                    llik_curr_theta[i][t][j] = setup.models[
                        i
                    ].llik(  # this one should be unnecessary ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        for i in range(setup.nexp):
            theta_hist[i][m] = theta[m, theta_unravel[i], delta[i][m].ravel()].reshape(
                setup.ntemps, setup.ns2[i], setup.p
            )
            pred_curr[i][:] = setup.models[i].eval(
                impa.tran_unif(
        ...
        tbar[:] = (theta[m] * theta_ext.reshape(setup.ntemps, setup.nclustmax, 1)).sum(
            axis=1
        )
                ...
                for i in range(setup.nexp):
                    ...
                    if False:
                        sw_alpha[:] = sw_alpha + (
                            ...
                            - 0.5
                            * (setup.ny_s2[i] * np.log(s2[i][m])).sum(axis=1)[sw.T[0]]
                            - 0.5 * sse_curr[i][sw.T[0]].sum(axis=1)
                            # for t_1
                            - invgamma_logpdf(
                            ...
                            + 0.5
                            * (setup.ny_s2[i] * np.log(s2[i][m])).sum(axis=1)[sw.T[1]]
                            + 0.5 * sse_curr[i][sw.T[1]].sum(axis=1)
                        )
                for tt in sw[
                    np.where(sw[np.log(uniform(size=setup.nswap_per)) < sw_alpha])[0]
                ]:
