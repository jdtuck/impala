...
def func_prediction_plot(
    ...
    if parent_y_lb is not None:
        plt.fill_between(
            actual_x, parent_y_lb, parent_y_ub, color="lightgrey", label=r"$\theta^*$"
        )
    if theta0_y_lb is not None:
        plt.fill_between(
            actual_x, theta0_y_lb, theta0_y_ub, color="lightblue", label=r"$\theta_0$"
        )
    plt.fill_between(
...
def ptw_prediction_plots_pool(setup, calib_out, path, mcmc_use, ylim=None, alpha=0.05):
    ...
    pred_theta_raw = [
        np.empty([mcmc_use.shape[0], setup.ys[i].shape[0]]) for i in range(setup.nexp)
    ]
    ...
    if ylim == "constant":
        m1 = min(
            [pp.min() for pp in pred_theta_quant_lb] + [pp.min() for pp in obs_stress]
        )
        m2 = max(
            [pp.max() for pp in pred_theta_quant_ub] + [pp.max() for pp in obs_stress]
        )
        ylim = [m1, m2]
...
def ptw_prediction_plots_hier(setup, calib_out, path, mcmc_use, ylim=None, alpha=0.05):
    ...
    for i in range(setup.nexp):
        ...
        pred_theta0_raw[i] = setup.models[i].eval(
            sc.tran_unif(
                calib_out.theta0[mcmc_use, 0], setup.bounds_mat, setup.bounds.keys()
            ),
            pool=True,
            nugget=True,
        )
    ...
    if ylim == "constant":
        m1 = min(
            [pp.min() for pp in pred_thetap_quant_lb] + [pp.min() for pp in obs_stress]
        )
        m2 = max(
            [pp.max() for pp in pred_thetap_quant_ub] + [pp.max() for pp in obs_stress]
        )
        ylim = [m1, m2]
    ...
    for i in range(setup.p):
        for j in range(setup.p):
            if i == j:
                ...
                plt.subplot2grid((setup.p, setup.p), (i, j))
                contour_list = [
                    kde_contour(x[:, j], x[:, i], 1 - alpha) for x in theta0_unst_list
                ]
                for k in range(n):
                    plt.contour(
                        contour_list[k]["X"],
...
def get_bounds(
    ...
    sub_dat = pd.DataFrame(
        {k: theta_parent_native[k][rank][idx_rank] for k in theta_parent_native.keys()}
    )
...
def get_best_sse(results_csv, write_path):
    ...
    rank_sse = np.argsort(theta_parent_native["sse"])
    sub_dat = pd.DataFrame(
        {k: theta_parent_native[k][rank_sse][0] for k in theta_parent_native.keys()},
        index=[0],
    )
