...
class PTW_Yield_Stress(BaseModel):
    ...
    def value(self, edot):
        ...
        eArg2 = (
            (eps * mp.p * mp.theta)[ind] / (mp.s0 - tau_y)[ind] / (np.exp(eArg1) - 1.0)
        ...
        scaled_stress[ind2] = +tau_s[ind2] - (tau_s - tau_y)[ind2] * np.exp(
            -eps[ind2] * mp.theta[ind2] / (tau_s - tau_y)[ind2]
        )
...
class MaterialModel(object):
    ...
    def update_state(self, edot, dt):
        ...
        cond = edot > edotcrit
        # if any(cond):
        self.state.T = self.state.T + cond * chi * self.state.stress * edot * dt / (
    ...
    def compute_state_history(self, strain_history):
        ...
        results[0] = np.array(
            [times[:, 0], state.strain, state.stress, state.T, state.G, state.rho]
        ...
        for i in range(1, Nhist):
            ...
            results[i] = np.array(
                [times[:, i], state.strain, state.stress, state.T, state.G, state.rho]
            )  # np.repeat(state.rho, nrep)])
...
def generate_strain_history(emax, edot, Nhist):
    tmax = emax / edot
    strains = np.linspace(0.0, emax, Nhist)
    nrep = len(edot)
    times = np.empty((nrep, Nhist))
    # for i in range(nrep):
    #    times[i,:] = np.linspace(0., tmax[i], Nhist)
    times = np.linspace(0.0, tmax, Nhist)
    ...
    return dict(
        (["times", times.T], ["strains", strains], ["strain_rate", strain_rate.T])
    )
